...
const Post = ({post, addComment, comments, auth:{ user }, profilePosts, deletePost, addLike, addRise}) => {
    ...
    // drag and drop
    const [isDaraged, setIsDaraged] = useState(false)
    const [draggedBy, setDraggedBy] = useState(0)
    const kipper = useRef()
    const handleDragStart = (e) =>{
        let translatedBy = kipper.current.parentNode.style.transform
        setIsDaraged(true)
        translatedBy && setDraggedBy(parseInt((translatedBy.split(", ")[1]).replace("px)","")))
        // effect
    }
    const handleDragEnd = () =>{
        console.log(draggedBy)
        if(draggedBy < -25){
            addRise(post._id)
            setDraggedBy(0)
        }
    ...
    return (
        <DragDropContext onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
            <Droppable droppableId={post._id} type="POST">{(provided) => (
                <div {...provided.droppableProps} ref={provided.innerRef} className={risedPost?"post blured-background rised":"post blured-background"}>
                    {provided.placeholder}
                    {/* behind post */}
                    <div className="behind-post"></div>
                    {post._id &&
                    <div ref={dropdownRef} className={isDaraged ? "dropdown d-none": "dropdown"}>
                    ...
                    <Draggable key={post._id} draggableId={post._id} index={0}>{(provided) => (
                        <div {...provided.draggableProps} {...provided.dragHandleProps} ref={provided.innerRef} className="kipper">
                            <div ref={kipper} className="inner">
                                <div className="head">
