...
const Post = ({post, addComment, comments, auth:{ user }, profilePosts, deletePost, addLike, addRise}) => {
    ...
    const { text } = formData
    const [postKipperDestroy, setPostKipperDestroy] = useState(false)
    // Post liked or not
    ...
    return (
        <div className={risedPost?"post blured-background rised":"post blured-background"}>
            ...
            {post._id &&
            <div ref={dropdownRef} className={postKipperDestroy ? "dropdown destroy" : "dropdown"}>
            ...
            <div onClick={()=>setPostKipperDestroy(true)} className={postKipperDestroy ? "kipper destroy" : "kipper"}>
                <div className="inner">
                    <div className="head">
                        <div className="off-user">
                            <Link to={`/profile/${post.user}`}>
                                <div className={postKipperDestroy ? "avatar destroy" : "avatar"}>
                                    <img src={post.avatar === "default"?defaultImg:post.avatar} alt="avatar" />
                                </div>
                            </Link>
                            <div className="user-info">
                                <div className="inline">
                                    <span className={postKipperDestroy ? "text-bold underline destroy" : "text-bold underline"}><Link to={`/profile/${post.user}`} title={post.name}>{userNameHandler(post.name,post.feeling ? post.feeling.length > 0 : false)}</Link></span>
                                    <label className={postKipperDestroy ? "destroy" : ""}>{post.feeling && (<p> is feeling&nbsp;<span>{ post.feeling }</span></p>)}
                                    </label>
                                </div>
                                <span className={postKipperDestroy ? "date underline destroy" : "date underline"} title={post.date}>{sinceDate(post.date)} {post.location &&
                                    (<p className={postKipperDestroy ? "destroy text-bold svg-icon" : "text-bold svg-icon"}>
                                        <svg viewBox="0 0 512 512">
                                                <g>
                                        ...
                                        <span className={postKipperDestroy ? "destroy" : ""}>{post.location}</span>
                                    </p>)}
                                </span>
                            </div>
                        </div>
                ...
                {post.text && 
                <div className={postKipperDestroy ? "body destroy" : "body"}>
                ...
                <div className="footer">
                    <div className="head">
                        {post.likes && 
                        <div onClick={()=>addLike(post._id)} className={postKipperDestroy ? "like react svg-icon destroy" : "like react svg-icon"}>
                        ...
                        <div onClick={()=>handleDropDown(post._id, 'commentSection')} className={postKipperDestroy ? "comment react svg-icon destroy" : "comment react svg-icon"}>
                            <svg  viewBox="0 0 60 60">
                                <g>
                        ...
                        {post.rises && 
                        <div onClick={()=>addRise(post._id)} className={postKipperDestroy ? "rise react svg-icon destroy" : "rise react svg-icon"}>
                    ...
                    {!profilePosts && <div className="head">
                        <div className={postKipperDestroy ? "share react svg-icon destroy" : "share react svg-icon"}>
                            <svg viewBox="0 0 512.001 512.001">
                                <g>
                                    <g>
